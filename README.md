...
## 项目概述
本项目是一个基于 Web 的地理空间数据计算与可视化系统，主要用于渔网点数据的多指标综合评价计算和空间插值分析。系统采用前后端分离架构，前端使用 Leaflet.js 进行地图可视化，后端使用 Node.js + Express 提供 API 服务，Python 脚本执行复杂的数值计算。
...
- 显示 QGIS 导出的静态图层列表
- 可以控制图层的显示/隐藏
![图层信息模式界面1](img/1-1.png)
![图层信息模式界面2](img/1.png)

#### 1.3 编辑计算模式（左右面板）
根据模式（编辑/计算）显示不同的内容：
**编辑模式**：
- **编辑功能**：显示所有数据信息，双击可编辑数据
![编辑模式界面1](img/2-1.png)
![编辑模式界面2](img/2-2.png)
![编辑模式界面3](img/2-3.png)
![编辑模式界面4](img/2-4.png)
...
**计算模式**：
![计算模式界面](img/3-1.png)
- **步骤1：选择算法**：熵权法，G1，组合权重-罗，组合权重-博弈，加权求和
![步骤1选择算法](img/3-2.png)
- **步骤2：配置字段和参数**：选择参与计算的数值字段（f1, f2, f3, f4 等）根据选择的计算方法配置相应参数
![步骤2配置字段和参数](img/3-3.png)
- **步骤3：选择归一化方式**：Min-Max标准化，Z-score标准化
![步骤3选择归一化方式](img/3-4.png)
- **执行计算**：点击"计算"按钮执行计算任务
![执行计算界面1](img/3-5.png)
![执行计算界面2](img/3-6.png)
![执行计算界面3](img/3-7.png)
![执行计算界面4](img/3-8.png)
- **可视化功能**：显示所有历史计算任务，每个计算结果可以：
  - 勾选/取消勾选显示在地图上
  - 调整透明度（0-100%）
  - 展开/折叠可视化选项
  - 查看数据详情
...
- 颜色根据数值大小映射
- 可调整点的大小（半径）
![点符号图-6px](img/dian-6.png)
![点符号图-6px](img/dian-6-max.png)
![点符号图-12px](img/dian-12-max.png)
**热力图**：
- 使用 Leaflet.heat 插件渲染
- 根据数据点密度和数值强度生成热力图
- 可调整半径（影响范围）和模糊度（平滑程度）
![热力图-半径20px](img/re-20px.png)
![热力图-半径20px透明度50%](img/re-20px-50%.png)
![热力图效果1](img/re.png)
![热力图效果2](img/re2.png)
![热力图效果3](img/re3.png)
**阈值分类**：
...
- 每个等级用不同颜色表示
- 可调整分级数量（3-10级）
![阈值分类-3级](img/yuzhi-3.png)
![阈值分类-10级](img/yuzhi-10.png)
